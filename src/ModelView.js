...
function ModelCamera({ model, setPredictions }) {
  ...
  const onReady = React.useCallback(
    (images) => {
      ...
      //  const predictions = await model.estimateSinglePose(nextImageTensor, {flipHorizontal: false}); // for single pose detection
      const predictions = await model.estimateMultiplePoses(nextImageTensor, {flipHorizontal: false}); // for multiple poses.
        // const predictions = await model.classify(nextImageTensor);
        console.log(predictions) // you can access this like an array 
